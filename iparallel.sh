...
echo "Parallel streams;Bandwidth, Gbits/sec;Retransmitted Segments, %;-;Transfer, GBytes" >> $OUT
# ------------------------------------------------------------------------------------------------
p=1
TEST="TCP-P$p"
echo Testing $TEST
...
echo $p";"$(awk -F"]" '/sender$/ { print $2 }' $FILE | awk -F" " '{ print $5 ";=" $7 "/(" $3 "*1024*1024*1024/131072);;" $3 ";" }')$(awk -F" " '/CPU/ { print $4 }' $FILE) >> $OUT
sleep $PAUSE
# ------------------------------------------------------------------------------------------------
for ((p=2; p<=16; p++))
do
...
iperf3 -c $1 -V -t $TIME -P $p -i 0 -f g -l $PACKET --logfile $FILE > /dev/null 2>&1
echo $p";"$(awk -F"]" '/SUM.*sender$/ { print $2 }' $FILE | awk -F" " '{ print $5 ";=" $7 "/(" $3 "*1024*1024*1024/131072);;" $3 ";" }')$(awk -F" " '/CPU/ { print $4 }' $FILE) >> $OUT
sleep $PAUSE
done
# --------------------------------------------------------------------------------------------------
